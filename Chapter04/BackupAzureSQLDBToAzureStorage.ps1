...
Set-AzCurrentStorageAccount -StorageAccountName $storageaccountname -ResourceGroupName $resourcegroupname | Out-Null
...
$export = New-AzSqlDatabaseExport -ResourceGroupName $resourcegroupname -ServerName $sqlserver.Split('.')[0] -DatabaseName $database -StorageUri $bloblocation -AdministratorLogin $credentials.UserName -AdministratorLoginPassword $credentials.Password -StorageKeyType StorageAccessKey -StorageKey $storageaccountkey.Value[0].Tostring() 
