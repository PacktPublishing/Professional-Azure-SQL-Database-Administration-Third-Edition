## Deleted database on cross instance
...
While (1)
		...
		Write-Host "Restoring deleted Database $deletedDatabaseName as $targetDatabaseName on $targetInstanceName to the time $restoretime"
        
        #restore the database to point in time
	    $restore = Restore-AzSqlInstanceDatabase -FromPointInTimeBackup -ResourceGroupName $resourcegroupname -InstanceName $managedInstanceName -Name $deletedDatabaseName -PointInTime $restoretime -DeletionDate $deletedDatabase.DeletionDate -TargetInstanceDatabaseName $targetDatabaseName   -TargetResourceGroupName $targetResourceGroupName -TargetInstanceName $targetInstanceName 
...
##.\SQLMI_DeletedDatabaseOnCrossInstancePITR.ps1 -resourcegroupname Packt -subscriptionId 6ee856b5-yy6d-4bc1-a901-byg5569842e1 -managedInstanceName packtsqlmi -deletedDatabaseName toystore -targetDatabaseName toystorepitr_restored -TargetResourceGroupName Packt -TargetInstanceName packsqlmi1