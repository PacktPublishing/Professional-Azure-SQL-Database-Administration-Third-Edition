...
{
    $profile = Import-AzContext -Path $AzureProfileFilePath
...
Get-AzSqlElasticPool -ElasticPoolName $ElasticPoolName -ServerName $SqlServer -ResourceGroupName $ResourceGroup -ErrorVariable notexists -ErrorAction SilentlyContinue
...
$CreateElasticPool = @{
  ...
  New-AzSqlElasticPool @CreateElasticPool;
...
Set-AzSqlDatabase -ResourceGroupName $ResourceGroup -ServerName $SqlServer -DatabaseName $db -ElasticPoolName $ElasticPoolName
...
$epdbs = Get-AzSqlElasticPoolDatabase -ElasticPoolName $ElasticPoolName -ServerName $SqlServer -ResourceGroupName $ResourceGroup
...
Set-AzSqlDatabase @RemoveDbsFromPool;
}
...
Remove-AzSqlElasticPool @RemovePool -Force;
